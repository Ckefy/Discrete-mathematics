...
board::board(const board &other) = default; //конструктор по умолчанию
...
board::board(int size) { //принимает размер и делает некое расположение
...
size_t board::size() const { //const значит, что нельзя менять члены класса того объекта, откуда вызван
    return _size;
}

bool board::is_goal() const { //является ли верным решением нынешняя доска
    return b == goal;
}

size_t board::hamming() const { //блоки не на своих местах
...
bool board::is_solvable() const {
    ...
    } //число инверсий - большие числа стоят раньше чем мелкие

    for (size_t i = 0; i < v.size(); i++) {
        if (!v[i]) {
            cnt_inv += 1 + i / _size; //было исправлено 4 на _size
        }
    }
    return !(cnt_inv & 1); //если нечетная сумма, то значит нет решений
}
...
} //расстояние от клетки до ее нужного состояния = abs(x - x) + abs(y - y)
//функция возвращает такую сумму всех клеток
...
void board::init_goal() { //создает нужное решение - массив заполненый числами от 1 до n^2 и пустота в последней клетке
...
void board::find_zero() { //находит где сейчас пустая клетка
